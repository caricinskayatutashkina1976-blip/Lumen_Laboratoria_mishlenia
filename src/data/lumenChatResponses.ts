...
export function getGeneralLumenResponse(question: string): string {
  ...
  if (!q) {
    return 'Сначала напиши вопрос. Даже короткая фраза поможет понять, где застрял.';
  }

  if (/не понял|не понимаю|объясни проще|не ясно|запутал/.test(q)) {
    return 'Ничего страшного. Давай проще. Сначала найдём, что известно, а потом — что нужно найти.';
  }

  if (/действи|слож|вычест|умнож|раздел|плюс|минус/.test(q)) {
    return 'Чтобы выбрать действие, нужно понять связь между числами. Если одинаковое число повторяется несколько раз — часто помогает умножение. Если нужно узнать остаток — вычитание. Если нужно разделить поровну — деление.';
  }

  if (/домашн|домашк|задач/.test(q)) {
    return 'Напиши условие задачи полностью. Я не буду сразу давать ответ, а помогу разобрать её по шагам.';
  }

  if (/провер|мой ответ|верно|правильн/.test(q)) {
    return 'Напиши свой ответ и коротко объясни, как ты рассуждал. Я помогу проверить ход решения.';
  }
  ...
  return 'Я пока не понял вопрос точно. Попробуй написать так: что известно, что нужно найти или на каком шаге ты остановился.';
}
