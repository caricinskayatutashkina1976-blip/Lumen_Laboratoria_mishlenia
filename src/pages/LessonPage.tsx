...
import { Grade6ReadinessCheck } from '../components/Grade6ReadinessCheck/Grade6ReadinessCheck';
...
import { ReviewBeforeTopic } from '../components/ReviewBeforeTopic/ReviewBeforeTopic';
...
import { getGrade6PrerequisiteRecommendation } from '../data/grade6Readiness';
...
export function LessonPage() {
  ...
  const mapLink = isGrade6 ? '/map?grade=6' : '/map';
  const showTrainingRecommendation = getTotalErrorCount(progress.errorStats) > 0;

  const grade6PrerequisiteRec =
    isGrade6 && topic
      ? getGrade6PrerequisiteRecommendation(topic.slug, getTopicProgress, progress)
      : null;
  ...
  if (!unlocked) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-12 text-center">
        ...
        <Link to={mapLink} className="mt-4 inline-block text-lumen-blue hover:underline">
          Карта тем
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
      <Link to={mapLink} className="text-sm text-lumen-silver transition-colors hover:text-lumen-blue">
        ← К карте тем
      </Link>
      ...
      {isGrade6 && (
        <div className="mb-8 space-y-6">
          <ReviewBeforeTopic topic={topic} />
          <Grade6ReadinessCheck topicSlug={topic.slug} />
          {grade6PrerequisiteRec && (
            <NextStepRecommendation recommendation={grade6PrerequisiteRec} />
          )}
        </div>
      )}
      ...
      {showTrainingRecommendation && !isGrade6 && (
        <section className="mb-8">
      ...
      {showTrainingRecommendation && isGrade6 && !grade6PrerequisiteRec && (
        <section className="mb-8">
          <NextStepRecommendation
            recommendation={getLumenRecommendation('lesson', { topicSlug: topic.slug })}
          />
